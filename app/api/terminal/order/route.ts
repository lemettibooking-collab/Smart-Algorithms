...
import type { TerminalExecutionErrorCode, TerminalPlaceOrderRequest } from "@/src/shared/model/terminal/contracts";
...
export async function POST(req: Request) {
  const rl = rateLimitOr429(req, { keyPrefix: "api:terminal-order-place", max: 120, windowMs: 10 * 60_000 });
  if (!rl.ok) return rl.res;

  let body: Partial<TerminalPlaceOrderRequest> | null = null;
  try {
    body = (await req.json()) as Partial<TerminalPlaceOrderRequest>;
  } catch {
  ...
  const response = await getTerminalExecutionAdapter().placeOrder(body);
  if (!response.ok) {
