...
import type { TerminalCancelOrderRequest, TerminalExecutionErrorCode } from "@/src/shared/model/terminal/contracts";
...
export async function POST(req: Request) {
  const rl = rateLimitOr429(req, { keyPrefix: "api:terminal-order-cancel", max: 120, windowMs: 10 * 60_000 });
  if (!rl.ok) return rl.res;

  let body: Partial<TerminalCancelOrderRequest> | null = null;
  try {
    body = (await req.json()) as Partial<TerminalCancelOrderRequest>;
  } catch {
  ...
  const response = await getTerminalExecutionAdapter().cancelOrder(body);
  if (!response.ok) {
